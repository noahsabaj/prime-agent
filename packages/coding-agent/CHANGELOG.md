...
- Fixed background subprocesses opening console windows on Windows. Daemon, worker, kernel, git, clipboard, package-manager, and tool-version launches now set `windowsHide`; the console editor, the CLI relaunch, and other windows the user is meant to see are unchanged ([#735](https://github.com/PrimeIntellect-ai/prime-agent/issues/735)).
